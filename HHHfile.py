...
def handle_resend_search_text(m):
    ...
    if len(query) < 2:
        bot.send_message(uid, "❌ Rubuta akalla haruffa 2 ko 3 ko 4, misali (Mash, masheki, mas).")
        return

    # 1️⃣ DUBA KO USER YA TABA SIYA KOMAI
    ...
    if total_bought == 0:
        # ❌ BAI TABA SIYA BA → FITA GABA DAYA
        user_states.pop(uid, None)
        bot.send_message(
            uid,
            "❌ <b>Baka taɓa siyayya ba.</b>\n"
            "Je ka siya fim a bangaren siyayya.",
            parse_mode="HTML"
        )
        return

    # 2️⃣ DUBA IYAKAR SAKE TURAWA
    used = conn.execute(
    ...
    # 3️⃣ NEMO FIM DAGA WANDA YA TABA SIYA
    rows = conn.execute("""
        SELECT DISTINCT
            um.movie_id,
            m.title
    ...
    # 4️⃣ IDAN BABU MATCH → CI GABA DA JIRA
    if not rows:
        bot.send_message(
            uid,
            "❌ A cikin fina-finan da ka taɓa siya babu fim mai wannan suna.\n\n"
            "Yi ƙoƙarin tuna sunan fim din ka sake gwadawa.\n"
            "Ina jiranka… 😊"
        )
        # ⚠️ KAR A CIRE STATE
        return

    # 5️⃣ AN SAMU → CIRE STATE SANNAN A NUNA
    user_states.pop(uid, None)
...
# ---------- FLEXIBLE TITLE + PRICE PARSER (EXTENDED) ----------
def parse_title_price_block(text_block):
    ...
    for raw in lines:
        ...
        # case: price only line (100, N100, ₦100, -100)
        if re.fullmatch(r"[-:]?\s*(?:₦|N)?\s*\d+", line, re.IGNORECASE) and pending_title:
        ...
        # otherwise treat as title waiting for price
        pending_title = line
...
def find_best_match(title, candidates):
    ...
    for i, c in enumerate(candidates):
        ...
        if t in fn:
            matches.append(i)
        elif first and first in tokens:
            matches.append(i)
...
def process_weak_finalize(uid):
    ...
    if not parsed:
        bot.send_message(
            uid,
            "❌ FORMAT BA DAI DACE BA\n\n"
            "Misali masu aiki:\n"
            "Fashin banki - 200\n"
            "Fashin banki 200\n"
            "Fashin banki N200\n"
            "Fashin banki\n200\n\n"
            "Ka gyara rubutun ka sake aikawa."
        )
        return
    ...
    for item in parsed:
        ...
        if isinstance(idx, list):
            msg = "❌ Na rikice, fina-finai sun yi kama:\n\n"
            for i in idx:
                msg += f"- {stored_files[i]['file_name']}\n"
            msg += "\nKa banbance sunan sosai ka sake rubutawa."
...
def send_weekly_list(msg):
    ...
    for m in items:
        ...
        # 👉 BUTTON DIN FILM + SUNA
        kb.row(
            InlineKeyboardButton(
                f"➕ Add Cart— {title}",
                callback_data=f"addcart:{mid}"
            ),
            InlineKeyboardButton(
                f"💳 BUY— {title}",
                callback_data=f"buy:{mid}"
    ...
    if all_ids:
        kb.add(
            InlineKeyboardButton(
                "🎁 BUY ALL",
                callback_data="buyall_week:" + ",".join(all_ids)
            )
        )
...
def admin_support_flow(m):
    ...
    if stage == "wait_order_id":
        ...
        items = conn.execute(
            "SELECT movie_id FROM order_items WHERE order_id=?",
            (text,)
        ).fetchall()

        movie_ids = [r["movie_id"] for r in items]
        ...
        bot.send_message(
            ...
            f"Tura <b>/sendall</b> domin a sake tura fina-finai.",
            parse_mode="HTML"
        )
        return
    ...
    if stage == "gift_message":
        ...
        data["stage"] = "gift_movie"
        bot.send_message(
            m.chat.id,
            "🎬 Rubuta <b>SUNAN FIM</b> (kamar sendmovie):",
            parse_mode="HTML"
        )
        return
    ...
    if stage == "gift_movie":
        ...
        if row:
            file, title = row["file_id"], row["title"]

        # SERIES
        if not file:
            row = conn.execute(
                "SELECT file_id, 'WANNAN FREE (KYAUTA) MUKA BAKU, Ku kalla🫶🏻🥰' AS title FROM series_items WHERE file_name LIKE ?",
                (f"%{q}%",)
            ).fetchone()
            if row:
                file, title = row["file_id"], row["title"]
        ...
        if not file:
            row = conn.execute(
                "SELECT file_id, 'WANNAN FREE (KYAUTA) MUKA BAKU, Ku kalla🫶🏻🥰' AS title FROM hausa_series_items WHERE file_name LIKE ?",
                (f"%{q}%",)
            ).fetchone()
            if row:
                file, title = row["file_id"], row["title"]
        ...
        if not file:
            row = conn.execute(
                "SELECT file_id, 'WANNAN FREE (KYAUTA) MUKA BAKU, Ku kalla🫶🏻🥰' AS title FROM weekly WHERE file_name LIKE ?",
                (f"%{q}%",)
            ).fetchone()
            if row:
                file, title = row["file_id"], row["title"]
        ...
        if not file:
            ...
            bot.send_message(
                m.chat.id,
                "❌ Ba a samu fim da wannan suna ba."
            )
            return
        ...
        bot.send_message(
            m.chat.id,
            f"🎁 <b>An kammala</b>\n"
            f"An tura kyauta zuwa:\n"
            f"User ID: <code>{data['gift_user']}</code>",
            parse_mode="HTML"
        )
...
def admin_sendall_cmd(m):
    ...
    for mid in items:
        ...
        for t in ["movies", "series_items", "hausa_series_items", "weekly"]:
            try:
                row = conn.execute(
                    f"SELECT file_id FROM {t} WHERE id=?",
                    (mid,)
                ).fetchone()
                if row:
                    file = row["file_id"]
    ...
    bot.send_message(
        uid,
        "🙏 Muna ba da haƙuri akan matsalar da aka samu.\n"
        "Ga fina-finan ka, mun sake turo su.\n"
        "Mun gode da hakuri ❤️"
    )
...
def build_unpaid_orders_view(uid, page):  
    ...
    for oid, amount, title in rows:  
        name = title if title else "Group orders (ba fim 1 ba)"  
        short = name[:15] + "…" if len(name) > 30 else name  
...
def build_paid_orders_view(uid, page):
    ...
    if total == 0:
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton("🎬 LAST PURCHASE", callback_data="my_movies"))
        kb.add(InlineKeyboardButton("⤴️ KOMA FARKO", callback_data="go_home"))
    ...
    for (title,) in rows:
        name = title or "Group order(ba fim 1 ba)"
        text += f"• {name} — ✅ Success\n"
